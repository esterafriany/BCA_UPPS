		...
		<div class="row">
			<div class="col-xs-11">
				<ul class="nav nav-pills nav-justified thumbnail" style="border: 1px solid rgba(255, 255, 255, 0);">
					...
					<li>
						<button id="clickButton" type="submit" name="Shuttle_Button" value="ShuttleBus" class="btn btn-primary btn-circle btn-xl"><i class="lnr lnr-bus"></i>
						</button> Shuttle Bus
					</li>
					<li>
...
<div class="container">
  ...
  $files = glob('./uploads/'.$_SESSION["InputProgramName"].'/'.$_SESSION["InputNamaAngkatan"].'/Shuttle/*');
  ?>
<!-- Modal -->
  <div class="modal fade" id="myFiles" role="dialog">
    <div class="modal-dialog">
    <?php
    $nextFile= $_SESSION["JumlahFileShuttle"]+1;
     ?>
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
        ...
        <table class="table table-hover">
        		<?php
        		if ($_SESSION["JumlahFileShuttle"]<1)
        		{
        			?>
        			<tr>
        		...
        		else
        		{
        			...
        			for ($i=0; $i < $_SESSION["JumlahFileShuttle"] ; $i++)
        			{
        			?>
        			<tr>
        			<?php
	       			$namaFile =str_replace('./uploads/'.$_SESSION["InputProgramName"].'/'.$_SESSION["InputNamaAngkatan"].'/Shuttle/','',$files[$i]);
	       		?>
	       			<td>
		...
		<font style="text-align:right;">
			<h6>
			<input type="hidden" id="Filenumber" name="JumlahFileShuttle" value="<?=$_SESSION["JumlahFileShuttle"]+1?>" />
...
$(function ()
{
    var FileDuplicates = <?php Print($_SESSION["JumlahFileShuttle"]+1); ?>;

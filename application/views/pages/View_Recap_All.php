			...
			foreach($Rekap_All as $all)
			{
				...
				<tr>
                    ...
                    $bookStatus = array(
                            ...
                            $all->status_konsumsi,
                            $all->status_hotel,
                            $all->status_shuttlebus,
                            $all->status_busbandara
                        );
                        
                        $status=false;
                        if(in_array("booked",$bookStatus))
                        {
                            if(!in_array("Pending",$bookStatus) && !in_array("rejected",$bookStatus) && !in_array("approved",$bookStatus))
                            {
                                $status=true;
                            }
                        }
					...
					<?php 
						...
						if($sign_days == "-" && $status==true)
						{
							echo "background-color:#279727;color:white;";
						}else if($num_days+1  >= 6  && $sign_days == "-" ){
							echo "";
							
						}else if(($num_days+1 >= 4 && $num_days+1 <= 5 ) & $sign_days == "-"){
							echo "background-color:#ebeb4f";
							
						}else if(($num_days+1 >= 0 && $num_days <= 3) && $sign_days == "-"){
							echo "background-color:#d42727;color:white;";
							
						}else if($sign_days == "+"){
							echo "background-color:#2196F3;color:white;";
						}
					?>
